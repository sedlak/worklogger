...
public class WorklogsBundle {
	...
	private CompanySpecifics specifics;
	private SortedMap<String, List<Worklog>> worklogsMap;

	public WorklogsBundle(CompanySpecifics specifics, SortedMap<String, List<Worklog>> worklogsMap) {
		this.specifics = specifics;
		this.worklogsMap = worklogsMap;
	}

	public CompanySpecifics getSpecifics() {
		return specifics;
	}
	...
	public int getWorklogsInSyncCount() {
		return worklogsMap.values().stream().mapToInt(
				list -> list.stream().mapToInt(w -> w.isInSync() == null || w.isInSync() ? 1 : 0).sum()).sum();
	}
	...
	public int getSecondsForDay(String date){
		return worklogsMap.getOrDefault(date, new ArrayList<>()).stream().mapToInt((list -> list.getTimeSpentSeconds())).sum();
	}
}
